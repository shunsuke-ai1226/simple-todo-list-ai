...
## 7. Webアプリとしてデプロイした場合の設定（重要）

アプリをVercelやNetlifyなどにデプロイした場合、**エラー 400: redirect_uri_mismatch** が発生します。
これを解決するには、デプロイしたURLをGoogle Cloud Consoleに追加する必要があります。

### 手順

1.  [Google Cloud Console](https://console.cloud.google.com/) にアクセスします。
2.  左メニューの「**認証情報**」をクリックします。
3.  作成したOAuth 2.0 クライアント IDをクリックします（鉛筆アイコンで編集できます）。
4.  **承認済みの JavaScript 生成元**セクションで：
    *   「**URI を追加**」をクリックします。
    *   デプロイしたURLを入力します（例: `https://simple-todo-list-ai.vercel.app`）
    *   ※ `http://localhost:5173` はそのまま残しておきます（ローカル開発用）
5.  **承認済みのリダイレクト URI**セクションでも：
    *   「**URI を追加**」をクリックします。
    *   同じURLを入力します（例: `https://simple-todo-list-ai.vercel.app`）
6.  画面下部の「**保存**」をクリックします。
7.  数分待ってから、デプロイしたアプリで再度同期を試してください。

### 複数の環境に対応する場合

開発環境、ステージング環境、本番環境など、複数のURLを使う場合は、それぞれを追加してください：

- `http://localhost:5173` （ローカル開発）
- `https://your-app-staging.vercel.app` （ステージング）
- `https://your-app.vercel.app` （本番）

### エラーが出る場合の確認事項

- ✅ URLに末尾のスラッシュ `/` がついていないか確認
- ✅ `http://` と `https://` が正しいか確認
- ✅ 保存後、数分待ってから再試行
- ✅ ブラウザのキャッシュをクリアして再試行
